...
meaningful order, and they do not advertise their creation time to a casual
reader. The timestamp is recoverable, so this hides nothing from anyone holding
this library. Do not treat an identifier as a secret or a capability.
...
The format is fixed. Do not rebind the internal alphabet or suffix width: doing
so silently invalidates every identifier already stored.
...
calls rather than expect the value back unchanged.

* Allocation

One namespace holds at most =identifier-base= identifiers per second, one per
seed, so allocation checks for collisions. The library does not know where
identifiers are stored, so the caller supplies that knowledge:
...
=identifier-space-exhausted= for a genuinely full second. Entropy quality only
spreads identifiers across seeds; it never affects correctness. Bind
=*random-index-function*= to make allocation reproducible under test.

=occupied-p= runs while the allocation lock is held, so it must not call back
into this library.

* Reservations

A generated identifier is reserved in this process until it is released, so two
threads never receive the same one while the first has yet to persist it.
...
second and nothing more. Reservations are process-local: they coordinate
threads, not processes. Concurrent processes sharing storage must make
=occupied-p= authoritative.
...
and decode modulo that bound. A suffix above the bound cannot have come from
